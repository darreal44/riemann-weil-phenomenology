...
$Q_L$: a smallness bound on the silence $\delta_p=v^{\!\top}T_pv$ and a largeness bound on the coupling $\kappa_p=\|T_pv-(v^{\!\top}T_pv)v\|$, uniformly in the window. The first linear readings ($e^{-7p}$, $e^{-4p}$) were local slopes; in the prime weight $w=p\log p$ the measurements read $-\ln\delta_p\approx0.19\,s(\chi)\,w$ and $-\ln\kappa_p\approx c(\mu)\,w^2$ (Section~6). Both quantities are pairings of $v$ against the prime tower
...
The measured slope of $-\ln\delta_p$ in $w$ is stable from $\mu=8$ to $30$; the values at fixed $p$ are not --- $\delta_7$ rises by four orders of magnitude between $\mu=8$ and $16$ (notebook \S71) --- so any statement uniform in $L$ must be about the slope, not about a fixed bound $e^{-7p}$. The statement
