...
\]
a Dirichlet kernel. Under RH the explicit formula reads $Q=\sum_{\gamma>0}2\,\hat c(\gamma)\hat c(\gamma)^{\!\top}$. On $V$ this is verified as a matrix identity once the truncated zero sum is completed by the density tail and the boundary term $C_G/G$, $C_G=2\Lambda(\mu)/(\pi L\sqrt\mu)$: the residual is $O(1/G^2)$ (notebook \S42--50, note \emph{squares47}), and the full $47\times47$ difference is an Arb enclosure containing $0$ in every entry (notebook \S65).
