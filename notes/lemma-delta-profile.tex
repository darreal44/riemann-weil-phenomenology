...
\bibitem{Chuk} X.~Zhu, \emph{Weil positivity in compact windows: a
finite reduction, certified two-sided bounds, and a Landau--Widom decay
law}, arXiv:2608.24827 (2026). [Author and title corrected 2026-09-16;
the entry was misattributed to ``M.~Chuk''; PDF not recompiled.]
\end{thebibliography}
...
\section*{Addendum --- cheap locks, 1 September}

Two of the three proposed locks, run.

\paragraph{A character already in hand.} $\chi_{-8}$ at $\mu=22$ has
rungs $\ell=(25.12,\,11.50)$, spacing $13.6$ at level $11.5$ --- inside
the $[0,15)$ band $10.9\pm1.5$ of the thirty-three-rung collapse. One
more $L$-function on the same profile, not a new architecture.

\paragraph{Synthetic rigid $\Gamma$.}
\texttt{code/delta\_synthetic.py}: $Q_\Gamma$ on a $2\pi$-unfolding
(no desert, no $\chi$) at $\mu=11$ produces spacings $7$--$9$ nats,
not $11$--$16$. Universality across the fifteen $L$-functions is
\emph{not} blindness to the spacing law of $\Gamma$. A2 as written
(``any discrete set, same $\Delta$'') fails this test. The lemma
stays: one profile for the Weil forms of the family, not for every
restriction to a discrete set.

The $L$-variation at fixed $\chi$ is the lock that still needs a
depth-adequate pair of windows on a slow character (now available:
$\chi_{-20}$ at $\mu=22$ and $\mu=32$).

\section*{Addendum --- $L$-variation at fixed $\chi$, 1 September}

$\chi_{-8}$, same engine, three windows:

\begin{center}\small
\begin{tabular}{lccc}
\toprule
$\mu$ & rungs $\ell$ & spacing $\Delta$ & band of the profile \\
\midrule
11 & $(9.56,\,0.41)$ & $9.15$ & $[0,15):\ 10.9\pm1.5$ \\
16 & $(16.34,\,4.59)$ & $11.75$ & $[0,15)$ on the shallow rung \\
22 & $(25.12,\,11.50)$ & $13.62$ & $[15,40):\ 13.5\pm1.5$ \\
\bottomrule
\end{tabular}
\end{center}
The spacing tracks the \emph{level}, not the window width. That is A1
measured at fixed arithmetic. Combined with the $2\pi$-comb (which
produces $\Delta\approx7$--$9$, off the profile) the two cheap locks
split: A1 holds, A2 as ``any $\Gamma$'' fails, A2 as ``any Weil form
in the family'' still stands.
\end{document}
