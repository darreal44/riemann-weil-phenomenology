...
def run(mu, NB, dps, DEG, K=8):
    ...
    # echelles cumulatives : P+A, puis + tours une a une
    subsets = [('P+A', [])]
    for i in range(len(primes)):
        subsets.append(('P+A+' + '+'.join(str(pp) for pp in primes[:i+1]), primes[:i+1]))
    for name, plist in subsets:
        Sc = mp.matrix(NP, NP)
        for n in range(NP):
            for m in range(NP):
                Sc[n,m] = Pm_[n,m] + Am[n,m] + mp.fsum(Tm[pp][n,m] for pp in plist)
        Ec = mp.eigsy(Sc, eigvals_only=True)
        print(f"  {name:16s} : vp basses = {[mp.nstr(Ec[k],3) for k in range(4)]}")
    E, V = mp.eigsy(S)
