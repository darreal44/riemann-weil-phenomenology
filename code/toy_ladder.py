...
def run(mu, NB, NPANEL, DEG, REMOVE=set(), POLE=True):
    t0 = time.time()
    L = mp.log(mu)
    ...
    while x <= int(mp.e**L + 1e-9):
        ...
        for q in [2,3,5,7,11,13,17,19,23]:
            if y % q == 0:
                ...
                while y % q == 0: y //= q
                break
        if p and y == 1 and p not in REMOVE: ppts.append((mp.log(x), mp.log(p)/mp.sqrt(x)))
        x += 1
    ...
    for n in range(NB+1):
        for m in range(n, NB+1):
            ...
            v = (W02 if POLE else 0) - (F0/2*CR + WRi) - Wp
            S[n,m] = v; S[m,n] = v
    tmat = time.time()-t0
...
if __name__ == '__main__':
    ...
    REMOVE = set(int(x) for x in sys.argv[5].split(',')) if len(sys.argv) > 5 and sys.argv[5] != '-' else set()
    POLE = (len(sys.argv) <= 6 or sys.argv[6] != 'nopole')
    run(mu, NB, NPANEL=5*NB+20, DEG=DEG, REMOVE=REMOVE, POLE=POLE)
