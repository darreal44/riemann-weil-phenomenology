#!/usr/bin/env python3
"""Schur Neumann at μ=3.5. Prime 3 enters. T infinite.

Q = A − ∑_{p<μ} χ(p)(log p)/√p Θ(log p). Same split as #61/#63.
...
def run() -> dict:
    ...
    bys = {r["name"]: r for r in snaps}
    q_ok = mu3_matches_qnm()
    pred_ok = (
        interior_primes(MU) == [2, 3]
        and theta_op_bound(LOG2, L) == 1.0
        and theta_op_bound(math.log(3.0), L) == 1.0
        and q_ok
        and (not by3[2]["s_lo_pos"])
        and by3[4]["s_lo_pos"]
        and by3[8]["s_lo_pos"]
        and (not by5[2]["s_lo_pos"])
        and by5[4]["s_lo_pos"]
        and (not by4[2]["s_lo_pos"])
        and by4[4]["s_lo_pos"]
        and bys["chi8"]["s_lo_pos"]
        and bys["chi7"]["s_lo_pos"]
        and bys["chi17"]["s_lo_pos"]
    )
    return {
        "mu": MU,
        "L": L,
...
def main() -> int:
    print(
        f"μ={MU} L=log {MU}; primes {{2,3}}; Neumann; T infinite; not Galerkin",
        flush=True,
    )
    data = run()
    print(
